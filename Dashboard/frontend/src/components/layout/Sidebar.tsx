...
import {
  ...
  ScrollText, BarChart3, Brain, PanelLeftClose, PanelLeft,
} from 'lucide-react';
...
export default function Sidebar() {
  ...
  return (
    ...
    >
      {/* Logo */}
      <div className="h-16 flex items-center justify-between px-3 border-b border-[var(--border-color)]">
          ...
          >
            {({ isActive }) => (
              <>
                ...
                {collapsed && (
                  <div className="absolute left-full ml-2 px-2.5 py-1.5 rounded-md bg-[var(--bg-elevated)] text-[var(--text-primary)] text-xs font-medium whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 shadow-lg border border-[var(--border-color)] z-50">
                    {label}
                  </div>
                )}
              </>
